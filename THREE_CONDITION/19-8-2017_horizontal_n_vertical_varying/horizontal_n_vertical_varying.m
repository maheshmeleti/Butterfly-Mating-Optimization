...
z = fitness(swarm(:,1,iter),swarm(:,2,iter),access(1),iter); %Fitness calculating from 3-peaks function
swarm(:,3,iter)=z;
UV = ones(bflies,1)*10;
swarm(:,4,iter)=UV;
% disp(swarm);
...
%  fprintf('tetha %d\n',tetha);
     ...
     z = fitness(swarm(:,1,iter),swarm(:,2,iter),access(iter),iter);
...
%   %UV updation
  for i=1:bflies
         UV(i)=b1*UV(i)+b2*fitness(swarm(i,5,iter),swarm(i,6,iter),access(iter),iter);
%          swarm(i,7,iter)=UV(i);
           swarm(i,4,iter)=UV(i);
